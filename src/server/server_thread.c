...
int
st_init_client (int cid, int32_t * client_max)
{
  int i;
  int response = 1;
...
//kill the banker
int
st_execute_banker (int cid, int *req)
{
  int i;
  int valid = 1;
  int enough = 1;
...
st_process_request (server_thread * st, int socket_fd)
{
  ...
  printf ("le serveur reçoit %d %d %d %d %d sur le socket %d\n", msg[0],
	  msg[1], msg[2], msg[3], msg[4], socket_fd);

  int32_t reponse[2];
  int waiting_time = 2;		//temps d'attente en secondes.
  //start = time(NULL);
  switch (msg[0])
    {
    case END:
      ...
      //on libère les resources du client
      for (int i = 0; i < num_resources; i++)
	{
	  allocation[msg[1]][i] = 0;
	}
